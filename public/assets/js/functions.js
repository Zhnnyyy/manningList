...
export const ExcelValidation = (emp) => {
  ...
  if (
    !emp.some(
      (obj) => obj.__EMPTY_2 && obj.__EMPTY_2.toUpperCase() == "EMPLOYEE NAME"
    )
  ) {
    swal({
      icon: "warning",
  ...
  for (let i = 0; i < Object.keys(emp).length; i++) {
    if (
      Object.keys(emp[i]).length >= 8 &&
      emp[i].__EMPTY_2 != "Employee Name"
    ) {
      if (emp[i].__EMPTY_2 && emp[i].__EMPTY_2 != "Employee Name") {
        ...
        newEmp.push({
          emp_Id: emp[i]["__EMPTY_1"],
          ..._splitName(emp[i]?.__EMPTY_2, emp[i].__rowNum__),
          position: emp[i]["__EMPTY_3"],
          assignment: emp[i]["__EMPTY_4"],
          region: emp[i]["__EMPTY_5"],
          rate: emp[i]["__EMPTY_6"],
          row: emp[i].__rowNum__ + 1,
        });
      }
    }
  }
